...
func save_terrain_gradients(terrainGradients: Dictionary) -> void:
...
func load_terrain_gradients() -> Array:
...
func calculate_terrain_gradients(grid: Dictionary, cell_size:Vector2i):
    var slope_gradients = {}
    for cell in grid:
        var slope = get_cell_slope(cell, cell_size)
        slope_gradients[cell] = slope

    return slope_gradients

func get_cell_slope(pos: Vector2i, cell_size: Vector2i) -> float:
...
func map_terrain(terrain: Node3D, cell_size: Vector2i):
...
#region diagnostics
func get_terrain_size(grid: Dictionary, cell_size:Vector2i) -> Vector2:
    var min_x = INF
    var max_x = -INF
    var min_z = INF
    var max_z = -INF
...
func print_terrain_info(grid: Dictionary, cell_size:Vector2i):
    var size = get_terrain_size(grid,cell_size)
